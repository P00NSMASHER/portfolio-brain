...
def validate_experiments():
    ...
    req(pin["source_revision"]=="c4be1a3e10a92b9255906b1600097fe6371ea929","unexpected experiment source revision")
    ...
    req(policy["automatic_external_act"] is False and policy["automatic_downstream_modify"] is False and policy["automatic_cash_spend"] is False and policy["automatic_model_calls"] is False,"experiment engine authority widened")
    req(portfolio["plan_count"]==20,"unexpected experiment plan count")
    req(portfolio["status_counts"]=={"READY_FOR_ISOLATED_EXECUTION":12,"HUMAN_APPROVAL_REQUIRED":6,"BLOCKED":2},"unexpected experiment status distribution")
    ...
    req(selected["status"]=="HUMAN_APPROVAL_REQUIRED" and selected["autonomous_execution_allowed"] is False,"selected external validation not human-gated")
    req("CUSTOMER_COMMUNICATION" in selected["approval_requirements"],"selected experiment missing customer-communication approval")
    req(selected["cost_boundary"]["external_messages_max"]==0 and selected["cost_boundary"]["autonomous_cash_spend_usd_max"]==0,"selected plan grants external authority")
    ...
    return {"plans":portfolio["plan_count"],"ready":12,"human_approval_required":6,"blocked":2,"outcomes":0,"selected_uncertainty":"UNC-EXTERNAL-PRJ-001"}
