...
def policy():return json.loads((ROOT/"action_engine"/"ACTION_POLICY.json").read_text())
def restore(output):
    ...
    def get(url):
        ...
        for attempt in range(3):
            if used>=6:raise RestoreError("action artifact request budget exceeded")
            used+=1
            req=urllib.request.Request(url,headers={"Accept":"application/vnd.github+json","Authorization":f"Bearer {token}","X-GitHub-Api-Version":"2022-11-28","User-Agent":"portfolio-brain-actions/1.0"},method="GET")
            try:
    ...
    with zipfile.ZipFile(io.BytesIO(raw)) as zf:
        if "action_state.json" not in zf.namelist():raise RestoreError("action_state.json missing")
        body=zf.read("action_state.json")
