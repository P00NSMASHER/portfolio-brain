...
def build_command_center_snapshot() -> dict[str, Any]:
    ...
    if portfolio_ceiling["cost_usd"] == 0 and portfolio_ceiling["model_calls"] == 0:
        alerts.append(
            {
                "severity": "INFO",
                "title": "Paid model/API spend remains locked",
                "detail": "Checked-in portfolio ceiling is $0 with zero model/API calls authorized.",
    ...
    snapshot = {
        ...
        "command_center_id": "portfolio-brain-command-center-v1",
        ...
        "operator_boundary": {
            ...
            "allowed": [
                ...
                "Review evidence-backed project, scheduler, cost, agent, Hunter, and validation-sprint status.",
...
<div class="shell">
<aside>
  <div class="brand"><div class="logo"></div><div>PORTFOLIO BRAIN<small>Command Center v1</small></div></div>
  <nav>
    <a href="#overview">Overview</a><a href="#alerts">Attention</a><a href="#projects">Portfolio</a>
    <a href="#agents">Agent Fleet</a><a href="#hunter">Hunter</a><a href="#cost">Cost & Controls</a>
    <a href="#workflows">Workflows</a><a href="#boundary">Authority Boundary</a>
  </nav>
...
<main>
  ...
  <section class="grid two">
    ...
    <div class="card">
      <div class="section-head"><div><h2>Validation Sprint</h2><p>{_e(sprint["sprint_id"])}</p></div>{_badge(sprint["status"],_status_tone(sprint["status"]))}</div>
      <p>{_e(sprint["purpose"])}</p>
      <div class="callout"><strong>Next admissible action</strong><p>{_e(sprint["next_admissible_action"])}</p></div>
      <p><strong>Primary:</strong> {_e(sprint["primary_commercial_track"])} &nbsp; <strong>Secondary:</strong> {_e(sprint["secondary_commercial_track"])}</p>
      <p><strong>Architecture policy:</strong> {_e(sprint["architecture_change_policy"])} until {_e(sprint["architecture_freeze_until"])}</p>
    </div>
  </section>
  ...
  <section class="grid two" style="margin-top:14px">
    ...
    <div class="card" id="cost">
      <div class="section-head"><div><h2>Cost Governor</h2><p>{_e(cost["mode"])}</p></div>{_badge("$0 PAID SPEND","good")}</div>
      <table><tbody>
  ...
  <section class="grid two" style="margin-top:14px">
    ...
    <div class="card">
      <div class="section-head"><div><h2>Sprint Guardrails</h2><p>Current validation-window constraints.</p></div></div>
      <ul>{guardrails}</ul>
    </div>
  </section>
