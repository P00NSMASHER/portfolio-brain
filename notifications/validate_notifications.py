...
def validate_notifications():
    ...
    req(kinds=={"HUMAN_APPROVAL_QUEUE","OPEN_HUMAN_BLOCKERS"},"unexpected current alert set")
    req(len(first["emitted_alerts"])==2 and first["active_alert_count"]==2,"current alert aggregation drifted")
    ...
    req(second["emitted_alerts"]==[] and len(second["suppressed_fingerprints"])==2,"duplicate/cooldown suppression failed")
    ...
    return {"current_signals":first["signal_count"],"current_emitted":2,"current_active":2,"dedup_suppressed_next_cycle":2,"delivery_channels":2,"authority":"NONE"}
